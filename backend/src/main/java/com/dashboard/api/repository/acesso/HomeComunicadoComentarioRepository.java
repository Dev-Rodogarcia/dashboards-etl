...
public interface HomeComunicadoComentarioRepository extends JpaRepository<HomeComunicadoComentarioEntity, Long> {
    @Query(value = """
            SELECT c.id AS id, c.corpo AS corpo, c.criado_em AS criadoEm, u.nome AS autorNome
    ...
    interface ComentarioProjection {
        Long getId(); String getCorpo(); java.time.Instant getCriadoEm(); String getAutorNome();
    }
}

...
public class ManifestosPerformanceSqlRepository {
    ...
    private static TemporalQuery temporalQuery(String nivel, Integer ano, Integer mes) {
        ...
        if (anoSeguro != null) {
            where.append("\n AND YEAR(data_criacao_date) = :anoTemporal");
        }

        return switch (nivelSeguro) {
            case "ano" -> new TemporalQuery("DATEFROMPARTS(YEAR(data_criacao_date), 1, 1)", "", null, null);
            ...
            default -> {
                if (mesSeguro != null) {
                    where.append("\n AND MONTH(data_criacao_date) = :mesTemporal");
                }
                yield new TemporalQuery("data_criacao_date", where.toString(), anoSeguro, mesSeguro);
            }
        };
    }

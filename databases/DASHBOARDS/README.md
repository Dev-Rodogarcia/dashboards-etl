...
```powershell
sqlcmd -S localhost,1433 -d DASHBOARDS -E -C -b -i databases\DASHBOARDS\migrations\V011__adicionar_escopo_filiais_usuario.sql
```

3. Validar o schema:

```powershell
sqlcmd -S localhost,1433 -d DASHBOARDS -U usuario_etl -P "<senha>" -C -b -i databases\DASHBOARDS\validation\001_validar_escopo_filiais_usuario.sql
```

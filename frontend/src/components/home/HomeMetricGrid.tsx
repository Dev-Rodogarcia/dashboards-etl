...
export default function HomeMetricGrid({ metrics }: { metrics: HomeMetric[] }) {
  return (
    <section aria-label="Resumo da Home" className="grid overflow-hidden rounded-[16px] border sm:grid-cols-4" style={{ backgroundColor: 'var(--color-card)', borderColor: 'var(--color-border)' }}>
      {metrics.map((metric) => {
        const Icon = METRIC_ICONS[metric.id as keyof typeof METRIC_ICONS] ?? Grid2X2;
        return (
          <article key={metric.id} className="flex min-w-0 items-center gap-2.5 border-b px-3 py-2 last:border-b-0 sm:border-b-0 sm:[&:not(:last-child)]:border-r" style={{ borderColor: 'var(--color-border)' }}>
